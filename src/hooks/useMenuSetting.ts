...
import { SIDEBAR_MINI_WIDTH, SIDEBAR_SHOW_TIT_MINI_WIDTH } from "/@/enums/app";
import { MenuModeEnum, MenuTypeEnum, TriggerEnum } from "/@/enums/menu";
...
export function useMenuSetting() {
  ...
  const getShowSidebar = computed(() => {
    return (
      unref(getSplit) ||
      (unref(getShowMenu) && unref(getMenuMode) !== MenuModeEnum.HORIZONTAL)
    );
  });
  ...
  const getIsTopMenu = computed(
    () => unref(getMenuType) === MenuTypeEnum.TOP_BAR
  );
